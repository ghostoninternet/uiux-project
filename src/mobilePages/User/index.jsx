    ...
    return (
        <Tabs className={'h-[calc(100vh-10rem)] relative'}>
            ...
            <Tab onClick={() => handleIconClick(0)}>
                {
                    activeId === 0 ?
                    <FaUser fontSize={25} color="#FABB18"/>:
                    <FaUser fontSize={25}/>
                }
            </Tab>
            <Tab onClick={() => handleIconClick(1)}>
                {
                    activeId === 1 ?
                    <FaHistory fontSize={25} color="#FABB18"/>:
                    <FaHistory fontSize={25}/>
                }
            </Tab>
            <Tab onClick={() => handleIconClick(2)}>
                {
                    activeId === 2 ?
                    <FaTrophy fontSize={25} color="#FABB18"/>:
                    <FaTrophy fontSize={25}/>
                }
            </Tab>
            </TabList>
        
            <TabPanel className={'h-[100%] overflow-y-scroll scrollbars-hidden'}>
                <div>
            ...
            <TabPanel className={'absolute top-[3.9rem] h-[calc(100%-3.9rem)] overflow-y-scroll scrollbars-hidden'}>
                <MobileAcitivity hiew={true}/>
            </TabPanel>
            <TabPanel className={'absolute top-[3.9rem] h-[calc(100%-3.9rem)] overflow-y-scroll scrollbars-hidden'}>
            <MobileAchivement hiew={true}/>
            </TabPanel>
        </Tabs>
    )
}
